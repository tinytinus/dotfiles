...
int main() {
    ...
    init_pair(3, COLOR_YELLOW, COLOR_BLACK);  // Dashes
    ...
    while(1) {
        ...
        if (ws) { fgets(workspace, sizeof(workspace), ws); pclose(ws); }
        workspace[strcspn(workspace, "\n")] = 0;
        
        // Time
        FILE *tm = popen("date +%H:%M", "r");
        if (tm) { fgets(time_str, sizeof(time_str), tm); pclose(tm); }
        time_str[strcspn(time_str, "\n")] = 0;
        
        // Volume
        FILE *vol = popen("pactl get-sink-volume @DEFAULT_SINK@ | awk '{print $5}' | tr -d '%'", "r");
        if (vol) { fgets(volume, sizeof(volume), vol); pclose(vol); }
        volume[strcspn(volume, "\n")] = 0;
        
        // Battery
        FILE *bat = popen("cat /sys/class/power_supply/BAT0/capacity", "r");
        if (bat) { fgets(battery, sizeof(battery), bat); pclose(bat); }
        battery[strcspn(battery, "\n")] = 0;
        
        // Backlight
        FILE *bl = popen("brightnessctl -m | cut -d, -f4 | tr -d '%'", "r");
        if (bl) { fgets(backlight, sizeof(backlight), bl); pclose(bl); }
        backlight[strcspn(backlight, "\n")] = 0;
        
        void print_line(int line, const char* label, const char* value, int color_pair) {
            int label_len = strlen(label);
            int value_len = strlen(value);
            int dash_count = width - label_len - value_len;
